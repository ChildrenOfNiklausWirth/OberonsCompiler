#include "T_D_EncodingAndDecoding.h"
...
void printReport(unsigned int IR, int op, int a, int b, int c, int mop, int ma,
		int mb, int mc, int wop, int wa, int wb, int wc) {
	printf("---------------------------------------\n");
...
int decodingTest(enum Report report, int op, int a, int b, int c) {
	...
	if (op > WRL) {
		if ((op == wop && wop == mop) && (c == wc && wc == mc)) {
			if (report == REPORT)
				printReport((unsigned int) IR, op, a, b, c, mop, ma, mb, mc,
						wop, wa, wb, wc);
			return 1;
		}
	} else {
		if ((op == wop && wop == mop) && (a == wa && wa == ma)
				&& (b == wb && wb == mb) && (c == wc && wc == mc)) {
			if (report == REPORT)
				printReport((unsigned int) IR, op, a, b, c, mop, ma, mb, mc,
						wop, wa, wb, wc);
			return 1;
	...
	if (report == ONLY_ERROR)
		printReport((unsigned int) IR, op, a, b, c, mop, ma, mb, mc, wop, wa,
				wb, wc);
	return 0;

}

int opTest(enum Report report, long op, long minA, long maxA, long minB,
		long maxB, long minC, long maxC) {
...
void risc_machine_tests() {
	...
	for (int op = 32; op <= 43; ++op) {
		if (op != 35 && op != 39) {
			if (opTest(ONLY_ERROR, op, 0, 1 << 4, 0, 1 << 4, -(1 << 7),
					1 << 8)) {
				passedTest++;
				printOperation(op);
				printf("TRUE\n");
